...
export function createStories(S, moodTail){
  return {
    child:{name:"꼬마 승객 루나",lvl:1,c:'child',
      get intro(){return "우주선에서 네 옆자리에 앉았던 꼬마, 루나다. 너를 알아보고 달려온다.\n\"오빠/언니다! 살아 있었구나! 나… 혼자였어.\"";},
      nodes:{start:{choices:[
    ...
    medic:{name:"의무관 하나",lvl:6,c:'medic',
      get intro(){return "우주선 의무실에서 봤던 하나가 구급상자를 안고 있다.\n\"다친 데 없어? 보자… 여기선 내가 도울 수 있는 게 이거뿐이라."+moodTail()+"\"";},
      nodes:{start:{choices:[
        ...
        {label:"\"그 약, 나 다 줘.\" 손을 뻗는다",m:'경쟁',line:"필요하면 빼앗기도 한다.",eff:function(){if(S.day<=3){S.rep-=1;return{ok:0,t:"하나가 황당해한다. \"…야, 같이 쓰는 거잖아.\" 머쓱하게 손을 거둔다.",foe:-7};}S.hunger+=16;if(S.wounds>0)S.wounds--;S.rep-=2;return{ok:1,t:"겁먹은 하나가 구급상자를 통째로 내준다. 받고 나니 영 개운치 않다. (먹을 것 +2, 목숨 회복)",foe:-9,next:'cursed'};}},
      ...
      blessed:{text:"하나가 작은 약 꾸러미를 쥐여준다. \"급할 때 써. 그리고… 다른 사람들한테도 나 여기 있다고 좀 전해줘.\"",choices:[
        {label:"\"꼭 전할게. 고마워.\"",m:'명예',line:"착한 일은 좋은 소문으로 돌아온다.",eff:function(){if(S.wounds>0)S.wounds--;S.rep+=2;return{ok:1,t:"약을 챙기고 하나의 이야기를 곳곳에 전했다. (목숨 회복)",foe:-9};}},
        {label:"약만 챙기고 말없이 간다",m:'경쟁',line:"받기만 하는 사람.",eff:function(){if(S.wounds>0)S.wounds--;S.rep-=1;return{ok:1,t:"약만 챙겨 떠났다. (목숨 회복)",foe:-9};}}
      ]},
    ...
    smith:{name:"정비사 도진",lvl:9,c:'smith',
      get intro(){return "우주선 정비사 도진이 잔해에서 쇳조각을 두드려 뭔가를 만들고 있다.\n\"오, 살아있었네! 이거 봐, 부서진 선체로 방패를 만들었어."+moodTail()+"\"";},
      nodes:{start:{choices:[
        ...
        {label:"\"그거 그냥 내놔.\" 빼앗으려 한다",m:'경쟁',line:"센 사람 것도 빼앗을 수 있다.",eff:function(){if(S.day<=3){S.rep-=1;return{ok:0,t:"도진이 어이없어한다. \"…우리 같은 배 탔잖아. 왜 이래?\" 괜히 미안해진다.",foe:-7};}var ok=Math.random()<0.4;if(ok){if(S.wounds>0)S.wounds--;S.rep-=2;return{ok:1,t:"방패를 낚아채 달아났다. 망치가 등 뒤를 스친다. (목숨 회복)",foe:-7};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"도진의 망치가 어깨를 때린다. (큰 상처)",foe:-1,next:'crushed'};}},
        {label:"\"같이 만들래?\" 손을 보탠다",m:'화친',line:"기술 가진 사람끼리 힘을 합치기.",eff:function(){var ok=S.rep>=-1;if(ok){if(S.wounds>0)S.wounds--;S.rep+=2;S.fear-=1;return{ok:1,t:"풀무질을 도우니 도진이 마음을 연다. 둘만의 작업장이 생겼다! (목숨 회복)",foe:-8};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"\"소문 안 좋은 너랑은 좀…\" 도진이 거리를 둔다. (작은 상처)",foe:0};}}
      ]},
    ...
    archer:{name:"정찰병 시아",lvl:8,c:'archer',
      ...
      nodes:{start:{choices:[
        ...
        {label:"먹을 걸 던져 시선을 끈다",m:'경쟁',line:"상대의 허기를 이용한다.",eff:function(){S.hunger-=16;var ok=Math.random()<0.7;if(ok){S.rep-=1;return{ok:1,t:"시아가 먹을 걸 줍는 틈에 빠져나왔다. (먹을 것 -2)",foe:-9};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"시아는 미끼를 무시하고 쏜다. (작은 상처, 먹을 것 -2)",foe:0};}}
      ]},
      ally:{text:"시아가 바위에서 내려온다. \"내가 위에서 망볼게. 넌 아래를 맡아. 우리 둘이면 사방을 다 보잖아.\"",choices:[
        {label:"\"좋아, 역할을 나누자.\"",m:'질서',line:"역할을 나누면 살 확률이 오른다.",eff:function(){if(S.wounds>0)S.wounds--;S.rep+=1;S.fear-=1;return{ok:1,t:"믿음직한 눈 하나가 늘었다. 시아가 함께 싸워준다! (목숨 회복)",foe:-9,recruit:1};}},
    ...
    farmer:{name:"겁먹은 승객 보라",lvl:4,c:'farmer',
      ...
      nodes:{start:{choices:[
        ...
        {label:"\"같이 나눠 먹자.\" 곁에 앉는다",m:'화친',line:"나누는 것이 평화의 출발.",eff:function(){S.hunger+=8;if(S.wounds>0)S.wounds--;S.rep+=2;return{ok:1,t:"보라가 떨리는 손으로 먹을 걸 나눠준다. \"고마워… 혼자선 너무 무서웠어.\" (먹을 것 +1, 목숨 회복)",foe:-5,next:'grateful'};}}
      ]},
      witnessed:{text:"상자를 메고 돌아서는데, 그 광경을 본 사람이 다가온다. \"약한 애 걸 빼앗네? 너도 똑같이 당해 봐.\"",choices:[
        {label:"\"덤빌 거면 덤벼.\"",m:'경쟁',line:"나쁜 소문은 모두를 적으로 만든다.",eff:function(){var ok=Math.random()<0.45;if(ok){if(S.wounds<MAXHP)S.wounds++;return{ok:1,t:"간신히 쫓아냈다. (작은 상처)",foe:-9};}if(S.wounds<MAXHP)S.wounds++;S.hunger-=16;return{ok:0,t:"상자를 도로 빼앗기고 다쳤다. (큰 상처, 먹을 것 -2)",foe:0};}},
    ...
    sleeper:{name:"잠든 승객",lvl:5,c:'sleeper',
      get intro(){return "한 승객이 빵빵한 식량 가방을 베고 풀밭에 곤히 잠들어 있다.\n주위엔 아무도 없다. 정말, 아무도."+moodTail();},
      nodes:{start:{choices:[
        {label:"발소리를 죽여 가방을 빼낸다",m:'경쟁',line:"아무도 안 볼 때, 사람은 어떻게 할까?",eff:function(){var ok=Math.random()<0.7;if(ok){S.hunger+=32;S.rep-=1;return{ok:1,t:"가방이 스르륵 빠져나왔다. (먹을 것 +4)",foe:-9};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"끈이 걸렸다. \"도둑이야!\" 잠이 깬다. (작은 상처)",foe:-2,next:'caught'};}},
        {label:"흔들어 깨워 사정을 말한다",m:'명예',line:"몰래 뺏는 건 떳떳하지 못하다.",eff:function(){var ok=S.rep>=-1;if(ok){S.hunger+=16;S.rep+=2;return{ok:1,t:"\"같은 배 탔던 사이잖아.\" 그가 먹을 걸 나눠준다. (먹을 것 +2)",foe:-6};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"강도로 알고 먼저 손이 나간다. (작은 상처)",foe:-1,next:'caught'};}},
      ...
      caught:{text:"잠이 깬 승객이 떨며 막대를 겨눈다. \"이젠… 같은 배 탔던 사람도 못 믿겠어.\"",choices:[
        ...
        {label:"맞붙는다",m:'경쟁',line:"한 번 시작된 싸움은 끝을 본다.",eff:function(){var ok=Math.random()<0.5;if(ok){S.hunger+=24;if(S.wounds<MAXHP)S.wounds++;return{ok:1,t:"몸싸움 끝에 가방을 빼앗았다. (먹을 것 +3, 작은 상처)",foe:-9};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"상대가 더 독했다. (큰 상처)",foe:0};}}
      ]}}},
    ...
    chief:{name:"무리를 모은 강윤",lvl:14,c:'chief',
      ...
      nodes:{start:{choices:[
        {label:"\"무리에 들어갈게.\"",m:'질서',line:"보호를 받는 대신 규칙을 따른다 — 작은 국가.",eff:function(){if(S.wounds>0)S.wounds--;S.fear-=2;S.rep-=1;return{ok:1,t:"무리에 들어갔다. 혼자보다 훨씬 안전하다. 대신 강윤의 규칙을 따라야 한다. (목숨 회복)",foe:-9,next:'inside'};}},
    ...
    deserter:{name:"무너진 승객",lvl:12,c:'deserter',
      get intro(){return "한 승객이 떨리는 손으로 날카로운 쇳조각을 쥐고 있다. 눈이 텅 비어 보인다.\n\"…다가오지 마. 다들 날 노린다고. 너도… 너도 그런 거지?\""+moodTail();},
      nodes:{start:{choices:[
        {label:"\"진정해. 나도 그냥 살려는 사람이야.\"",m:'화친',line:"같은 처지가 사람을 뭉치게 한다.",eff:function(){var ok=Math.random()<(0.45+S.rep*0.04);if(ok){S.rep+=2;if(S.wounds>0)S.wounds--;S.fear-=1;return{ok:1,t:"그가 쇳조각을 천천히 내린다. \"…미안. 너무 무서워서.\" (목숨 회복)",foe:-7};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"의심을 못 거둔다. 먼저 쇳조각을 휘두른다. (작은 상처)",foe:-1};}},
        ...
        {label:"먹을 걸 내밀며 천천히 다가간다",m:'명예',line:"두려움은 친절로 풀린다.",eff:function(){if(S.hunger<4)return{ok:0,t:"내밀 먹을 게 없다. 그가 더 사납게 경계한다.",foe:-1};S.hunger-=8;var ok=Math.random()<0.6;if(ok){S.rep+=3;S.fear-=2;return{ok:1,t:"먹을 걸 받아 든 그가 울먹인다. \"…고마워. 사람이 그리웠어.\" (먹을 것 -1)",foe:-8};}if(S.wounds<MAXHP)S.wounds++;return{ok:0,t:"손을 뻗는 순간 겁에 질려 쳐낸다. (작은 상처, 먹을 것 -1)",foe:-1};}}
      ]}}}
  };
}
