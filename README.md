...
동작 보존을 위해 **고치지 않고 남겨둔** 원본의 문제들이 있다. 배경과 수정 방법은
[`docs/TODO.md`](docs/TODO.md)에 정리돼 있다.

- **hp ±1/±2 무효 구문 16곳** — 대화 결과에 `(작은 상처)` / `(목숨 회복)`이 떠도
  실제로는 하트가 변하지 않는 경우가 있다. 원본의 shim이 변화량 3 이상일 때만
  반응했기 때문. 실제 효과를 주는 것은 **밸런스 변경**이라 별도 결정이 필요하다.
- **`moodTail()`이 항상 빈 문자열** — 날이 갈수록 NPC 소개문이 어두워지도록 만든
  장치가, 문자열이 로드 시점에 한 번만 평가되는 탓에 작동하지 않는다.
- **`sendPrompt()` 미정의** — 엔딩 화면의 두 번째 버튼이 호출하는 함수가 원본부터
  정의되어 있지 않다. 누르면 콘솔 에러가 나고 아무 일도 일어나지 않는다.
  (`다시 하기` 버튼은 정상)
